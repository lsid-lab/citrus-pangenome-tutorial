...
**`%3` は同時実行数の上限**です。9 本を一斉に流すと 9 × 32 = 288 CPU を同時に要求することになります。ユーザあたりの上限が設定されていない環境でも、共有クラスタを 1 人で占有しないよう絞っておくのが無難です。自分の割り当てとキューの混み具合に合わせて調整してください。
...
以下は**ある特定の環境での実測値**です。自分の環境に当てはめる前に、まず測定条件を確認してください。
...
これらは共同研究者の実測値です。最大でも 18 GB 程度なので、§5.7.4 の `--mem=24G` で十分な余裕があります。

#### この数字を読むときの注意

**1. 所要時間は出力先のストレージに強く依存します。**

pggb は `-D` / `--temp-dir` を指定しない限り、**中間ファイルを出力ディレクトリにそのまま書きます**。上の実測は出力先が NFS なので、wfmash の PAF や seqwish の中間ファイルの読み書きがすべてネットワーク越しでした。ノードローカルの SSD / NVMe に temp を逃がせる環境なら、短縮できる可能性があります。
...
> `-D` に渡すパスに**カンマを含めないでください**。seqwish が PAF ファイルの区切りと解釈します(pggb 側でエラーになります)。

**2. 「32 CPU」は論理 CPU の数です。**

測定に使ったノードは SMT が有効なので、`-t 32` が物理 16 コア相当だった可能性があります(Slurm の割り当て単位によります)。物理コアを 32 本使える環境なら、これより短くなります。

**3. ノードは共有されていました。**

他のジョブと I/O やメモリ帯域を分け合った状態での測定です。占有ノードならこれより速いことがあります。

つまり**この表は上限でも下限でもなく、「NFS 上で、共有ノードの論理 32 CPU で回したらこうだった」という 1 点の観測**です。自分の環境ではまず 1 染色体で測ってから、全体の計画を立ててください。
