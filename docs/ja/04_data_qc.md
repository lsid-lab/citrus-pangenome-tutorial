...
|---|---|---|---|---|---|---|---|
| CUN | 1 | 9 | 348.5 | 34.5 | 35.9 | ✅ PASS | 親 2 品種より 25-55 Mb 大きい → §4.6 |
...
CUN の 2 hap が他より大きいことは**事実として記録しておきます**が、これを「異常」と呼んでよいかは別の話です。§4.6 で改めて考えます。
...
**ただし、大きい/小さいのどちらも「それだけでは異常の証拠になりません」。** 近縁種間でもゲノムサイズは普通に 10-20% 違いますし、推定値自体にも幅があります。§4.6 で、この落とし穴を実例で扱います。
...
## 4.6 数字を「異常」と呼ぶ前に —— CUN の 2 hap が大きい件

§4.4 の表で、**CUN の 2 haplotype が親 2 品種より 25-55 Mb 大きい**ことに気づきます:

- CUN hap1: 348.5 Mb / CUN hap2: 358.0 Mb
- CKI・CKU の 4 hap: 303.4 - 323.9 Mb

これをどう読むか。**この章でいちばん練習してほしいのはここ**です。

### ステップ1: そもそも基準内に収まっている

`qc01_stats.sh` が使う柑橘の期待範囲は **290-370 Mb** なので、348.5 も 358.0 も**範囲内**です。6 haploid すべてが `PASS` になります。「他より大きい」ことと「基準を外れている」ことは別です。

### ステップ2: 同じデータセット内ではなく、既知の値と比べる

**温州みかんのゲノムサイズとして公表されている値**を並べてみます:

| 出典 | 報告されたサイズ |
|---|---:|
| Shimizu et al. (2017) 温州 draft (*Front Genet*) | 359.7 Mb |
| Kawahara et al. (2020) 温州 (MiGD) | 346 Mb |
| **本教材の CUN hap1 / hap2** | **348.5 / 358.0 Mb** |

**先行研究の値とぴたりと重なります。** つまり温州のアセンブリサイズは、温州として**ごく普通**です。

むしろ素直な読み方は、「温州が大きい」ではなく「**親 2 品種(そもそも別種です)のゲノムが相対的に小さい**」でしょう。近縁種間でゲノムサイズが 10-20% 違うのは、柑橘に限らずありふれたことです。

> **教訓**: 「同じデータセットの中で他より大きい」ことは、それ自体では異常の証拠になりません。**比べるべき相手は、手元の他サンプルではなく、その種について既に知られている値**です。§4.5 のポイント2 が言っているのは、まさにこのことです。

### ステップ3: サイズとは独立の観点 —— k-mer の重複度

アセンブリの総塩基数とは別に、**アセンブリ内部で配列がどれくらい重複しているか**という観点があります。本教材の開発過程で行われた k-mer 集計では、次の傾向が観察されました。

<!-- TODO: この解析の実施者名と解析条件(k の値、使用ツールとバージョン、対象が pmol 全体か chr のみか)を確認して明記する -->
...
> ⚠️ **この表の扱い**: これは**未公開の内部集計**であり、査読を経たものではありません。解析条件(k の値、使用ツールとバージョン、対象範囲)も確定していません。ここでは「こういう見方がある」という**手法の例示**として扱ってください。**Isobe et al. (2023) のアセンブリ品質に対する評価として読まないでください。**

CUN の 2 hap では、2 回出現する k-mer の割合が親系統の約 2 倍になっています。この差の説明としては、少なくとも次の 3 つが考えられます。

1. **温州のほうが反復配列が多い** —— 生物学的に本当の違い
2. **haplotype leakage** —— trio phasing で 2 hap を分離しきれず、同じ配列が両 hap に残った
3. **集計条件の違い** —— k の値、低複雑度領域のマスク、unplaced 配列を含むかどうか など

### ステップ4: 決着のつけ方を知っておく

大事なのは、**この 3 つを区別する手段が存在する**ことです。

そもそも Merqury (Rhie et al., 2020) は「アセンブリ内の k-mer 重複」を数えるツールではなく、「**生リード由来の k-mer データベース**とアセンブリを照合する」ツールです。trio のリードが揃っていれば、次のような**直接的な指標**が得られます:

- **QV** —— 塩基レベルの精度
- **completeness** —— リードの k-mer のうちアセンブリに含まれる割合
- **false duplication rate** —— **haplotype leakage の直接的な指標**
- **hap-mer blob plot** —— どちらの親由来の k-mer が、どちらの hap に入ったか

**haplotype leakage を主張するなら、本当に見るべきはこれら**です。リードは DDBJ DRA (PRJDB15866) から取得できますが、本教材は生リードを扱わないため、ここまでは行いません。したがって本教材は、**上の 3 つの仮説のどれが正しいかを判定しません**。

### なぜこの話を教材に入れるのか

**QC で最も難しいのは数字を出すことではなく、「その数字を異常と呼んでよいか」を判断すること**だからです。ここで身につけてほしい型は:

1. **観察する** —— CUN の 2 hap が他より大きい
2. **短絡しない** —— 「他より大きい = 異常」ではない。既知の値と比べれば、むしろ想定どおり
3. **別の観点を足す** —— k-mer 重複度には確かに差がある
4. **仮説を複数立てる** —— 生物学的な差 / leakage / 集計条件
5. **決着のつけ方を知っておく** —— trio リードを使った Merqury なら区別できる
6. **決着がつくまで断定しない** —— そのうえで、**下流にどう効くかは追跡しておく**

そして 6 が実務的に重要です。第6章では、CUN の hap が大きいことが **Jaccard 類似度を押し下げる**(§6.4.4)のを実際に目にします。原因が 1 であっても 2 であっても、**graph の解釈に効くという事実は変わりません**。だからここで記憶しておく価値があります。
...
**本教材では HiFi 生リードを扱わないため、Merqury は実行しません。** §4.6 で引用した k-mer 集計は Merqury のこの本来の使い方とは別物である点に注意してください(詳細は §4.6 の注記)。生リードは DDBJ DRA の BioProject `PRJDB15866`(§3.1)から取得できるので、興味があれば自分で試してみてください。
...
- **CUN の 2 hap は他より大きいが、温州として公表されているサイズ (346-360 Mb) の範囲内**。「他より大きい = 異常」と短絡しない
- k-mer の重複度には差があるが、原因(生物学的な差 / haplotype leakage / 集計条件)は**この教材のデータだけでは決まらない**。判定には trio リードを使った Merqury が必要
- ただし原因が何であれ、**第6章の Jaccard 類似度には効く**ので、観察として憶えておく
