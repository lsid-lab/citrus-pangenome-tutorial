...
```
  ...
  [CUN#1 = CKU 由来 | CUN#2 = CKI 由来]
```

### ハプロタイプは「親の染色体のコピー」ではない

CUN の 2 本のハプロタイプが**それぞれ片方の親に由来する**ことは確かです。**CUN#1 は父親 (CKU、九年母) 由来、CUN#2 は母親 (CKI、紀州) 由来**で、これは trio phasing (§2.5) によって保証されています。
...
- **CUN#1** = CKU_hap1 と CKU_hap2 の**モザイク**(父親の減数分裂で組み換わったもの)
- **CUN#2** = CKI_hap1 と CKI_hap2 の**モザイク**(母親の減数分裂で組み換わったもの)
...
乗換えは 1 本の染色体あたり**最低 1 回、通常は 1〜3 回**起こります。したがって CUN#1 の中には「CKU_hap1 に似ている区間」と「CKU_hap2 に似ている区間」が交互に現れ、**「CUN#1 ≈ CKU_hap1」のような 1 対 1 の対応は成立しません**。ゲノム全体で均せば、CUN#1 は CKU の 2 本とおおよそ半々の類似度を示します(染色体ごとの比率は乗換えの位置次第で大きくばらつきます)。

**曖昧なのは「親のどちらのハプロタイプか」であって、「どちらの親か」ではない**、という区別が重要です。組み換わった断片はすべて母親の 2 本のどちらかに由来するので、**親の特定は揺らぎません**。

そして、これは机上の注意書きではありません。**第7章の path 類似度解析で、この切り替わりが実際に観察できます**(§7.5.2)。染色体に沿って「CUN#1 は CKU_hap1 と CKU_hap2 のどちらに近いか」をプロットすると、組み換え点で乗り換わる様子が見えます。むしろ**それが見えることこそ、graph が pedigree と整合している証拠**になります。
...
子(温州)のリード R1: このk-merは親A由来 → 紀州由来ハプロタイプ (CUNphKi)
子(温州)のリード R2: このk-merは親B由来 → 九年母由来ハプロタイプ (CUNphKu)
```
...
| 品種 | アセンブリ ID | 内容 | 本教材の PanSN 名 |
|---|---|---|---|
| 温州 CUN | CUNphKu_r1.0 | 温州の**九年母 (**Ku**nenbo) 由来**ハプロタイプ | `CUN#1` |
| 温州 CUN | CUNphKi_r1.0 | 温州の**紀州 (**Ki**shu) 由来**ハプロタイプ | `CUN#2` |
| 紀州 CKI | CKIhap1_r1.0 | 紀州の haploid 1 | `CKI#1` |
| 紀州 CKI | CKIhap2_r1.0 | 紀州の haploid 2 | `CKI#2` |
| 九年母 CKU | CKUhap1_r1.0 | 九年母の haploid 1 | `CKU#1` |
| 九年母 CKU | CKUhap2_r1.0 | 九年母の haploid 2 | `CKU#2` |

> **ハプロタイプ番号の割り当てについて**: PanSN の `#1` / `#2` は**任意のラベル**で、どちらをどちらにするかに決まりはありません。本教材は **`CUN#1` = CUNphKu(父方・九年母由来)、`CUN#2` = CUNphKi(母方・紀州由来)** とします。`tables/samplesheet.tsv` の `hap1_path` / `hap2_path` の並びがこの割り当てを決めており、これが VCF の座標基準 (`-V CUN#1`、§5.2) や第6・7章の pedigree 検証の向きまで一貫して効きます。**他所の解析結果と突き合わせるときは、まず番号の割り当てが同じかを確認してください。**
...
**発見1**: CUN#1(九年母由来)は、CKI よりも CKU の 2 本に明確に近い
- ただし CKU_hap1 / CKU_hap2 のどちらか一方に張り付くのではなく、染色体に沿って**切り替わる**(§2.4 の組み換え)

**発見2**: CUN#2(紀州由来)は、CKU よりも CKI の 2 本に明確に近い
- こちらも CKI_hap1 / CKI_hap2 のモザイクとして見える
